...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {/* 🔥 MOBILE MENU */}
      {open && (
        <div className="md:hidden bg-dark-900 border-t border-cyan-500/20 px-4 py-4 space-y-4">
          <NavLink to="/" label="HOME" onClick={() => setOpen(false)} />
          <NavLink to="/image" label="IMAGE DETECTION" onClick={() => setOpen(false)} />
          <NavLink to="/video" label="VIDEO DETECTION" onClick={() => setOpen(false)} />
          <NavLink to="/about" label="ABOUT" onClick={() => setOpen(false)} />

          <Link
            to="/image"
            onClick={() => setOpen(false)}
            className="block btn-cyan text-center py-2 mt-2"
          >
            GET STARTED
          </Link>
        </div>
...
const NavLink = ({ to, label, onClick }) => {
  return (
    <Link
      to={to || '#'}
      onClick={onClick}
      className="block text-gray-300 hover:text-cyan-400 transition duration-300 relative group text-sm font-semibold tracking-wide"
    >
      {label}
